...
其原理是将对象的依赖关系由外部容器来管理和注入。这样，对象只需要关注自身的核心功能，而不需要关心如何获取依赖对象。它的目的是解耦组件之间的依赖关系，提高代码的灵活性、可维护性和可测试性。符合[**依赖倒转原则**](https://www.seven97.top/system-design/design-pattern/overviewofdesignpatterns.html#依赖倒转原则)
